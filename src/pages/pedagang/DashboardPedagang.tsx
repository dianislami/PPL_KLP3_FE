...
export default function DashboardPedagang() {
  ...
  const fetchPanen = async () => {
    try {
      ...
      setDataPanen(data.map((item: any) => ({
        ...item,
        id: item._id,
        nama: item.nama_komoditas,
        petani: item.user_id?.nama || 'Petani Unknown',
        jumlah: item.jumlah,
        satuan: 'Kg',
        harga: item.harga || 0,
        kualitas: item.kualitas || 'Grade A',
        lokasi: 'Aceh',
        tersedia: item.status !== 'Terjual',
        foto: item.foto || [],
      })));
    } catch (error) {
  ...
  const filtered = dataPanen.filter(p => {
    ...
    const matchFilter = filter === 'Semua' ? true : filter === 'Tersedia' ? p.tersedia : !p.tersedia;
    return matchSearch && matchFilter;
  });

  const totalTersedia = dataPanen.filter(p => p.tersedia).length;
  const totalNilai    = dataPanen.filter(p => p.tersedia).reduce((s, p) => s + p.jumlah * p.harga, 0);
    ...
    <div className="w-full min-h-screen bg-[#7a8c2e] flex flex-col">
      ...
      <div className="px-6 py-6 text-white">
        <div className="flex justify-between items-start mb-4">
          <div>
            <h1 className="text-4xl font-bold leading-tight">Hallo,<br />Pedagang</h1>
      ...
      <div className="mx-4 bg-white rounded-2xl shadow-md p-3 grid grid-cols-3 gap-2 z-10 relative -mb-3">
        <div className="text-center">
          ...
          <p className="text-xs font-bold text-gray-800">{totalTersedia} Produk</p>
        </div>
        ...
        <div className="grid grid-cols-4 gap-3 px-4 mb-6">
          {[
            ...
            >
              ...
              <span className="text-[11px] font-semibold text-[#5a6e1a]">{m.label}</span>
            </button>
          ))}
        </div>
          ...
          <div className="flex items-center gap-1 bg-gray-100 rounded-full p-1">
            <button
              ...
              className={`w-7 h-7 rounded-full flex items-center justify-center text-sm transition-all ${
                viewMode === 'grid' ? 'bg-[#7a8c2e] text-white shadow-sm' : 'text-gray-400'
              }`}
            >
              ⊞
            </button>
            <button
              ...
              className={`w-7 h-7 rounded-full flex items-center justify-center text-sm transition-all ${
                viewMode === 'list' ? 'bg-[#7a8c2e] text-white shadow-sm' : 'text-gray-400'
              }`}
            >
              ☰
            </button>
          ...
          <div className="grid grid-cols-2 gap-3 px-4">
            {filtered.map(p => (
              <button
                ...
                className="relative rounded-2xl overflow-hidden h-48 cursor-pointer active:scale-95 transition-all group text-left"
              >
                {/* Background */}
                ...
                <div className="absolute top-2 left-2">
                  <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${kualitasColor[p.kualitas] ?? 'bg-gray-100 text-gray-600'}`}>
                    {p.kualitas}
                  </span>
                </div>
                ...
                <div className="absolute top-2 right-2 bg-white/90 rounded-lg px-2 py-1">
                  <p className="text-[9px] text-gray-700 font-bold">
                    Rp {p.harga.toLocaleString('id-ID')}/Kg
                  </p>
                </div>

                {/* Info Bottom */}
                <div className="absolute bottom-0 left-0 right-0 px-3 py-2">
                  <p className="text-sm font-bold text-white truncate">{p.nama}</p>
                  <div className="flex justify-between items-center mt-0.5">
                    <p className="text-[10px] text-white/70 truncate">{p.petani}</p>
                    <p className="text-[10px] text-white font-semibold">{p.jumlah.toLocaleString()} Kg</p>
                  </div>
                </div>
              </button>
            ))}
            ...
            {filtered.map(p => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-gray-400 truncate">👨‍🌾 {p.petani} · 📍 {p.lokasi}</p>
                  <div className="flex items-center gap-2 mt-1.5">
                    <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${kualitasColor[p.kualitas] ?? 'bg-gray-100 text-gray-600'}`}>
                      {p.kualitas}
                    </span>
                    <span className="text-[10px] text-gray-500">{p.jumlah.toLocaleString()} Kg tersedia</span>
                  </div>
                </div>
