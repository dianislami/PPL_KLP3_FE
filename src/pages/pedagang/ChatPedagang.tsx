...
function ProdukBubble({ pesan, time }: { pesan: string; time: string }) {
  ...
  return (
    <div className="flex justify-end">
      <div className="flex flex-col items-end" style={{ width: 288 }}>
        <div
        ...
        <p className="text-[10px] text-gray-400 mt-1.5 pr-1">{time}</p>
      </div>
    </div>
  );
}

export default function ChatPedagang() {
  ...
  const locationState          = location.state as { produk?: any } | undefined;
  ...
  // Auto-send produk — sekali saja
  useEffect(() => {
    if (
      locationState?.produk &&
      !productSentRef.current &&
      user?.id &&
      contactId &&
      messages.length === 0
    ) {
      ...
      const productMessage =
        ...
        `🌾 ${p.nama}\n` +
        `💰 Rp ${p.harga?.toLocaleString?.('id-ID') || p.harga || '0'}/Kg\n` +
        `📊 ${p.kualitas}\n` +
        `⚖️ ${p.jumlah} Kg tersedia\n\n` +
        `${p.deskripsi ?? ''}`;

      chatAPI.sendMessage(user.id, contactId, productMessage)
  ...
  const fetchContactInfo = async () => {
    try { setContactInfo((await authAPI.getUserById(contactId || '')).data); } catch { /* silent */ }
  };

  const fetchChatMessages = async () => {
    try {
      setLoading(true);
      setMessages((await chatAPI.getChatMessages(user?.id || '', contactId || '')).data || []);
    } catch { /* silent */ }
  ...
  if (loading) {
    return (
      <div className="w-full min-h-screen bg-[#7a8c2e] flex items-center justify-center">
        <p className="text-white text-sm">Memuat chat...</p>
      </div>
    );
  }
