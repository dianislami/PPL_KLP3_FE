...
export default function DaftarChatPedagang() {
    ...
    <div className="w-full min-h-screen bg-[#7a8c2e] flex flex-col">
      ...
      <div className="mx-4 bg-white rounded-2xl shadow-md p-3 grid grid-cols-3 gap-2 z-10 relative -mb-3">
        <div className="text-center">
          ...
          <p className="text-xs font-bold text-gray-800">{chatList.length} Orang</p>
        </div>
        <div className="text-center border-x border-gray-100">
          <p className="text-[10px] text-gray-400">Online</p>
          <p className="text-xs font-bold text-green-600">{chatList.filter(c => c.online).length} Aktif</p>
        </div>
        <div className="text-center">
        ...
        {loading ? (
          ...
          <div className="flex flex-col divide-y divide-gray-50">
            {filtered.map(item => (
              <button
                key={item.id}
                onClick={() => navigate(`/chat-pedagang/${item.id}`)}
